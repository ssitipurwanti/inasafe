...
class Test_IO(unittest.TestCase):
    ...
    def test_vector_feature_count(self):
        ...
        for vectorname in ['exposure/lembang_schools.shp',
                           'exposure/tsunami_exposure_BB.shp',
                           'exposure/Padang_WGS84.shp',
                           'exposure/OSM_building_polygons_20110905.shp',
                           'test/OSM_subset.shp']:

            filename = os.path.join(TESTDATA, vectorname)
            ...
            key = vectorname.split('/')[-1]
            assert FEATURE_COUNTS[key] == N
    ...
    def test_reading_and_writing_of_vector_point_data(self):
        ...
        for vectorname in ['lembang_schools.shp',
            ...
            filename = os.path.join(TESTDATA, 'exposure', vectorname)
            layer = read_layer(filename)
    ...
    def test_analysis_of_vector_data_top_N(self):
        ...
        for vectorname in ['exposure/lembang_schools.shp',
            ...
            # Check results
            for N in [5, 10, 11, 17]:
                if vectorname == 'exposure/lembang_schools.shp':
                ...
                elif vectorname == 'exposure/tsunami_exposure_BB.shp':
    ...
    def test_vector_class(self):
        ...
        filename = os.path.join(TESTDATA, 'exposure', layername)
    ...
    def test_reading_and_writing_of_vector_polygon_data(self):
        ...
        filename = os.path.join(TESTDATA, 'test', vectorname)
        layer = read_layer(filename)
    ...
    def test_centroids_from_polygon_data(self):
        ...
        for vectorname in ['test/kecamatan_geo.shp',
                           'test/OSM_subset.shp']:
    ...
    def test_reading_and_writing_of_real_rasters(self):
        ...
        for rastername in ['test/Earthquake_Ground_Shaking_clip.tif',
                           'test/Population_2010_clip.tif',
                           'test/shakemap_padang_20090930.asc',
                           'test/population_padang_1.asc',
                           'test/population_padang_2.asc']:
    ...
    def test_no_projection(self):
        ...
        filename = os.path.join(TESTDATA, 'test', rastername)
        try:
            read_layer(filename)
        except RuntimeError:
            pass
    ...
    def test_nodata_value(self):
        ...
        for rastername in ['test/Population_2010_clip.tif',
                           'hazard/Lembang_Earthquake_Scenario.asc',
                           'hazard/Earthquake_Ground_Shaking.asc']:
    ...
    def test_bins(self):
        ...
        X = os.path.join(TESTDATA, 'test')
        for filename in ['%s/population_padang_1.asc' % X,
                         '%s/test_grid.asc' % X]:
    ...
    def test_get_bounding_box(self):
        ...
        for filename in ['test/Earthquake_Ground_Shaking_clip.tif',
                         'exposure/tsunami_exposure_BB.shp']:
    ...
    def test_layer_API(self):
        ...
        for layername in ['exposure/lembang_schools.shp',
                          'hazard/Lembang_Earthquake_Scenario.asc']:
    ...
    def test_polygon_area(self):
        ...
        # Realistic polygon
        filename = os.path.join(TESTDATA, 'test', 'test_polygon.shp')
        layer = read_layer(filename)
        geometry = layer.get_geometry()
        attributes = layer.get_data()
    ...
    def test_line_to_points(self):
        ...
        # Realistic polygon
        filename = os.path.join(TESTDATA, 'test',
                                'indonesia_highway_sample.shp')
        layer = read_layer(filename)
        geometry = layer.get_geometry()
        attributes = layer.get_data()
